...
const getStateByTestType = (type) => {
  for (const key of Object.keys(TEST_TYPE_TO_STATE)) {
    const mapValue = TEST_TYPE_TO_STATE[key];
    if (mapValue.includes(type)) {
      return key;
    }
  }
  throw new Error(`Test Type ${type} is not map to implemented state`);
};
...
class State {
  ...
  getItem() {
    throw new Error('getItem Method need to be implemented');
  }

  setItem(itemId) {
    throw new Error('setItem Method need to be implemented');
  }

  getCurrentState() {
    throw new Error('getCurrentState Method need to be implemented');
  }

  finishItem() {
    throw new Error('finishItem Method need to be implemented');
  }
...
class AgentContex {
  ...
  setItem(testType, itemId) {
    ...
    if (!setItem) {
      throw new Error(`State ${state} need to be implemente`);
    }
    setItem(this, itemId);
  }
