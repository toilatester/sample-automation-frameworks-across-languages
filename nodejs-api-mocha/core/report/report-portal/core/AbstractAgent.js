...
function createRequestObject(rawRequestBody, requestData, allowEmptyRequestData) {
  const isStartItemRequest = allowEmptyRequestData && Object.keys(rawRequestBody).length === 0;
  const isStartLaunchRequestWithParentId = requestData.id !== null || requestData.id !== undefined;
  if (isStartLaunchRequestWithParentId) {
    return requestData;
  }
  if (isStartItemRequest) {
    return {};
  }
