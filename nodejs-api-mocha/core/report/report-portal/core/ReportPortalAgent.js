...
class ReportPortalAgent extends AbstractAgent {
  ...
  get launchId() {
    return this.agentContex.state.getParentId(this.agentContex.launchItem);
  }

  setLaunchState(launchId) {
    this.agentContex.setItem(Constant.TEST_TYPE.LAUNCH, launchId);
  }

  setSuiteState(suiteId) {
    this.agentContex.setItem(Constant.TEST_TYPE.SUITE, suiteId);
  }

  setTestState(suiteId) {
    this.agentContex.setItem(Constant.TEST_TYPE.TEST, suiteId);
  }
  ...
  async finishAllAgentRequests() {
    ...
    const launchTempId = itemInfo.launchItem;
    // We will wait to finish all requests when finish launch
    if (launchTempId) return this.client.getPromiseFinishAllItems(launchTempId);
